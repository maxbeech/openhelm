...
def section_tool(section: str):
    ...
    All registered tools are wrapped with a per-call timeout to prevent
    indefinite hangs (60s default, 120s for heavy operations like screenshots).
    """
    def decorator(func):
        if is_section_enabled(section):
            return mcp.tool(with_timeout()(func))
        else:
            return func
    return decorator
