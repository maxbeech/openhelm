...
export function AreaChartViz({ data, config, columns, compact }: Props) {
  const height = compact ? 180 : 280;

  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 4, left: compact ? 0 : 8 }}>
        ...
        {config.series.map((s, i) => {
          const color = getChartColor(i, config.colors);
          return (
            <Area
              key={s.columnId}
              type="monotone"
