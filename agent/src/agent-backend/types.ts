...
export interface AgentRunResult {
  ...
  /**
   * True when the agent emitted a completion signal (e.g. Claude Code's
   * stream-json `result` event with a terminal stop_reason) and then fell
   * silent within the tail window. Backends that cannot detect natural
   * completion leave this undefined.
   */
  naturalCompletion?: boolean;
  /**
   * Per-tool invocation stats captured during the run. Used by the executor
   * to (a) persist into the run_tool_stats table for the runs-by-tool chart,
   * and (b) decide whether an MCP tool-missing error was recovered from by a
   * subsequent successful call against the same server.
   */
